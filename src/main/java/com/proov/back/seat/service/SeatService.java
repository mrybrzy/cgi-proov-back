package com.proov.back.seat.service;

import com.proov.back.seat.dto.SeatDto;
import com.proov.back.seat.dto.SeatMapper;
import com.proov.back.seat.entity.SeatEntity;
import com.proov.back.seat.repository.SeatRepository;
...
public class SeatService {
    ...
    public void bookSeat(Long seatId, Long movieId) {
        ...
        for (SeatDto seat : seats) {
            if (Objects.equals(seat.seatId(), seatId)) {
    ...
    public List<Integer> getRecommendedSeats(Long movieId, Integer capacity) {
        ...
        for (SeatDto seatDto : seats) {
            if (Boolean.FALSE.equals(seatDto.isBooked())) {
    ...
    private float getMiddleValue(List<List<Integer>> allCombinations) {
        ...
        return middleList.stream().mapToInt(Integer::intValue).sum();
    ...
    private boolean isConsecutive(List<Integer> toCheck) {
        for (int i = 1; i < toCheck.size(); i++) {
            if ((toCheck.get(i) != toCheck.get(i - 1) + 1) || (toCheck.get(i) % 10 != 0 && toCheck.get(i - 1) % 10 == 0)) {
                return false;
            }
        }
        return true;
